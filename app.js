...
// ==================== CONFIG ====================
const PARALLEL_WORKERS = 3; // Reduced for better compatibility
const BATCH_SIZE_PER_WORKER = 30;
...
async function tryBatchParallel(cookie, otpBatch, workerId) {
    ...
    for (let i = 0; i < otpBatch.length; i++) {
        try {
            ...
            // প্রতি ২০টি attempt এ একবার delay
            if (i % 20 === 0) {
    ...
    const workerPromises = chunks.map((chunk, index) =>
        new Promise(resolve => {
            ...
            setTimeout(async() => {
                ...
                resolve(result);
            }, index * 1000); // 1 second delay between worker starts
        })
    );
        ...
        workerPromises.forEach(promise => {
            promise.then(result => {
                ...
                if (result && !found) {
                    ...
                    resolve(result);
                } else if (completedWorkers === workers && !found) {
...
app.get('/', (req, res) => {
    res.json({
        ...
        example: {
            url: '/get-info?nid=1234567890&dob=1990-01-01'
        }
    });
});
...
app.get('/docs', (req, res) => {
    res.send(`
        ...
        <html>
        <head>
            <title>NID API Documentation</title>
            <style>
        ...
        <body>
            <h1>🇧🇩 Bangladesh NID API Documentation</h1>
            
            <div class="endpoint">
                <h3>GET /get-info</h3>
                <p>Get NID information by NID number and date of birth</p>
                <p><strong>Parameters:</strong></p>
                <ul>
                    <li><code>nid</code> - National ID number (required)</li>
                    <li><code>dob</code> - Date of birth in YYYY-MM-DD format (required)</li>
                </ul>
                <p><strong>Example:</strong></p>
                <code>/get-info?nid=1234567890&dob=1990-01-01</code>
            </div>
